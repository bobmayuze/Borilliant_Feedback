...
bool Mesh::Collapse() {
  ...
  // Get the average position of a and b
  a->Average(b);
  ...
  // make a copy of the traingles with b
  std::set<Triangle*> tmp = b->getTriangles();

  // remove all of the edges for triangles with b
  for (std::set<Triangle*>::iterator itr = tmp.begin(); itr != tmp.end(); itr++) {
    Triangle *t = *itr;
    Vertex *x = t->getVertex(0);
    Vertex *y = t->getVertex(1);
    Vertex *z = t->getVertex(2);
    RemoveEdge(x, y);
    RemoveEdge(x, z);
    RemoveEdge(y, z);
  }
  ...
  // If a triangle has a and b remove the triangle
  // If the triangle just has b, replace b with a
  // Remove vertex b
  for (std::set<Triangle*>::iterator itr = tmp.begin(); itr != tmp.end(); itr++) {
    Triangle *t = *itr;
    if (t->HasVertex(a)) RemoveTriangle(t);
    else {
      b->RemoveTriangle(t);
      if (t->getVertex(0) == b) t->SetVertex(0, a);
      if (t->getVertex(1) == b) t->SetVertex(1, a);
      if (t->getVertex(2) == b) t->SetVertex(2, a);
      a->AddTriangle(t);
    }
  }
  RemoveVertex(b);

  // Iterate through the a triangles, add edges from the triangle
  // Collect edges belonging to each vertex, recalculate the collected edges
  tmp = a->getTriangles();
  std::set<Edge*> collect;
  for (std::set<Triangle*>::iterator itr = tmp.begin(); itr != tmp.end(); itr++) {
    Triangle *t = *itr;
    Vertex *x = t->getVertex(0);
    Vertex *y = t->getVertex(1);
    ...
    AddEdge(x, z);
    AddEdge(y, z);
    CollectEdgesWithVertex(x, collect);
    CollectEdgesWithVertex(y, collect);
    CollectEdgesWithVertex(z, collect);
  }
  ReCalculateEdges(collect);
...
void Mesh::AddEdge(Vertex *a_, Vertex *b_) {
  ...
  if (itr == edges.end()) {
    ...
    if (method == "priority_queue") {
      if (!edges_pq.exist(ep)) edges_pq.push(ep);
    }
  }
