...
bool find_solution(int r, int c, char** board, char** tmp_board, int row, int col,
  ...
  for (int x = 0; x < row; ++x) {
    ...
    for (int y = 0; y < col; ++y) {
      ...
      if (y+length <= col && length <= row_occupied[x]) {
        ...
        // check2
        if (check1) {
          // std::cout<<x << ' ' << y <<" h ck 1 "<< used[used.size()-1].get_type()<<std::endl;
          for (int m = 0; m < length; ++m) {
      ...
      // std::cout<<x<< ' '<<y<<" 10h    "<< used[used.size()-1].get_type() << row_occupied[x] <<std::endl;
    }
  }
  ...
  for (int x = 0; x < row; ++x) {
    ...
    for (int y = 0; y < col; ++y) {
      ...
      // std::cout<<x<< ' '<<y<<"    10v "<<used[used.size()-1].get_type()<<std::endl;
    }
  }
