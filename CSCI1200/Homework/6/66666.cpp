...
bool find_solution(int r, int c, char** board, char** tmp_board, int row, int col,
  ...
  for (int x = 0; x < row; ++x) {
    ...
    for (int y = 0; y < col; ++y) {
      ...
      // check1
      if (y+length <= col+1 && length <= row_occupied[x]) {
      ...
      if (check_all && find_solution(x, y, board, tmp_board, row, col, row_occupied, col_occupied, ship, used)) {
        return true;
      }
    }
  }
  ...
  for (int x = 0; x < row; ++x) {
    ...
    for (int y = 0; y < col; ++y) {
      ...
      if (x+length <= row+1 && length <= col_occupied[y]) {
        for (int m = 0; m < length; ++m) {
...
int main(int argc, char* argv[]) {
  ...
  if (find_solution(0, 0, bd, tmp_bd, row, col, roc, coc, ship, used)) {
    found = true;
  ...
  for (int i = 0; i < row; ++i) {
    std::cout<< '|';
    for (int j = 0; j < col; ++j) std::cout << tmp_bd[i][j];
