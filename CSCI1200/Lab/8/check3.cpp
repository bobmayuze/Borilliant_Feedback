...
void print_grid(vector<vector<bool> > const& blocked_grid, vector<Point>& allpts, int start_x, int start_y) {
  ...
  for (int y=0; y<blocked_grid[0].size(); ++y) {
    for (int x=0; x<blocked_grid.size(); ++x) {
      ...
      if (x == start_x && y == start_y)
        ...
        cout << " X";
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 else if(x==0&&y==2)cout<<" .";
      else {
        for (unsigned int i = 0; i < allpts.size(); ++i) {
          if (x == allpts[i].x && y == allpts[i].y) {
            cout << " *";
            print = true;
            break;
          }
...
bool find_road(Point pt, int& num, vector<vector<bool> >& blocked_grid, vector<Point>& allpts) {
  allpts.push_back(pt);
  if (blocked_grid[pt.x][pt.y] == true){
    allpts.pop_back();
    return false;
  }
//  if (pt.x == 0 && pt.y == 0) {
...
//  }
  if (pt.x == 0) {
    for (unsigned int i = 1; i < pt.y; ++i) {
      if (blocked_grid[0][i] == true) {
        allpts.pop_back();
        return false;
      }
    }
    num++;
    for (unsigned int i = 0; i < pt.y; ++i) allpts.push_back(Point(pt.x-i,pt.y));
    return true;
  }
  if (pt.y == 0) {
    for (unsigned int i = 1; i < pt.x; ++i) {
      if (blocked_grid[i][0] == true) {
        allpts.pop_back();
        return false;
      }
    }
    num++;
    for (unsigned int i = 0; i < pt.x; ++i) allpts.push_back(Point(pt.x,pt.y-i));
    return true;
  }
 if (find_road(Point(pt.x-1, pt.y), num, blocked_grid, allpts) +
  find_road(Point(pt.x, pt.y-1), num, blocked_grid, allpts))
   return true;
  allpts.pop_back();
  return false;
...
int main(int argc, char* argv[]) {
  ...
  print_grid(blocked_grid, allpts, start_x, start_y);
  
  // Start here with your code...
  int num = 0;
  find_road(Point(start_x, start_y), num, blocked_grid, allpts);
  std::cout << "solution: " << num << std::endl;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   allpts.push_back(Point(0,0));
  print_grid(blocked_grid, allpts, start_x, start_y);
