...
bool find_road(Point pt, int& num, vector<vector<bool> >& blocked_grid) {
  if (blocked_grid[pt.x][pt.y] == true) return false;
  if (pt.x == 0) {
    for (unsigned int i = 1; i < pt.y; ++i) {
      if (blocked_grid[0][i] == true) return false;
    }
    num++;
    return true;
  }
  if (pt.y == 0) {
    for (unsigned int i = 1; i < pt.x; ++i) {
      if (blocked_grid[i][0] == true) return false;
    }
    num++;
    return true;
  }
  return  find_road(Point(pt.x-1, pt.y), num, blocked_grid) + find_road(Point(pt.x, pt.y-1), num, blocked_grid);
}
...
int main(int argc, char* argv[]) {
  ...
  int num = 0;
  find_road(Point(start_x, start_y), num, blocked_grid);
